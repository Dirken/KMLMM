...
ggplot(Components,aes(x=Components,y=R_squared)) + 
  geom_point(shape=15) + geom_line() + scale_x_continuous(breaks = c(1:9)) +
  labs(y = expression(R^{2})) + ggtitle(expression(paste("Components against  ", R^{2})))
...
model <- lm(as.matrix(Ytest2) ~ TIB - 1)
